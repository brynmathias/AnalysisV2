...
/*
*/  
TH1* his( std::string dir, TH1* input, Int_t nbins, Double_t xlow, Double_t xhigh ) {
  if ( !input ) { return 0; }
  TString name( TString( dir ) + "_" + TString( input->GetName() ) + "Clone" );
...
//   double reco_min = 1.e15;
  ...
  for ( Int_t i = 0; i < 1; ++i ) {
    ...
    // Open files
    std::stringstream ss;
    ss << "results/Ratio_QCD_AllPtBins_7TeV_Pythia_part0.root";
    TString name(ss.str());
	...
	dir << "Ratio" << at[iat];
	if ( j == 0 ) {
	  pre << dir.str() << "Pt" << pt[i] << ".0/GenHt" << eq << "Pre" << "_" << multi;
	  post << dir.str() << "Pt" << pt[i] << ".0/GenHt" << eq << "Post" << "_" << multi;
	  std::cout << pre.str() << std::endl;
	  std::cout << post.str() << std::endl;
	} else if ( j == 1 ) {
	  pre << dir.str() << "Pt" << pt[i] << ".0/Ht" << eq << "Pre" << "_" << multi;
	  post << dir.str() << "Pt" << pt[i] << ".0/Ht" << eq << "Post" << "_" << multi;
	  std::cout << pre.str() << std::endl;
	  std::cout << post.str() << std::endl;
	}
	
	// Create ratio histo
	TH1* denominator = his( dir.str(), (TH1*)file->Get(TString(pre.str())), 50, 200., 700. );
	TH1* numerator = his( dir.str(), (TH1*)file->Get(TString(post.str())), 50, 200., 700. );
	int rebin = 5;
