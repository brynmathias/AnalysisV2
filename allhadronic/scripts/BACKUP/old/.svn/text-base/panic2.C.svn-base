...
int panic2() {
  ...
  const Int_t nfile = 2;
  Int_t style[nfile] = { /*kOpenSquare,*/ kFullCircle, kFullSquare };
  ...
  // HT binning
  const int nht = 3;
  const double minht = 150;
  const double maxht = 450;
  double ht[nht];
  ...
  // -----------------------------------------------------------------------------
  // R(aT) vs pT for different HT bins
  if (0) {
    ...
    for ( int iat = 0; iat < 1; ++iat ) {
      ...
      TCanvas* canvas = new TCanvas(TString("RatioVsPt_"+ss_canvas.str()),
				    TString("RatioVsPt_"+ss_canvas.str()));
      canvas->SetFillColor(0);
      canvas->SetLineColor(0); 
      canvas->SetLogy();
      canvas->SetRightMargin(0.2);
	...
	for ( int iht = 0; iht < 4; ++iht ) {
	  ...
	  std::stringstream ss_histo;
	  ss_histo << "RatioVsHt_" << type[ifile] << "_AlphaT" << at[iat]/1000. << "_HT" << ht[iht];
 	  TH1D* histo = new TH1D(TString(ss_histo.str()),"",npt,minht,maxht);
	  
 	  for ( int ipt = 0; ipt < npt; ++ipt ) {
 	    if (anchor) { histo->SetBinContent(ipt+1,ratio[ifile][iat][ipt][iht]*scale[ifile][iat][ipt]); }
	    else { histo->SetBinContent(ipt+1,ratio[ifile][iat][ipt][iht]); }
 	    histo->SetBinError(ipt+1,error[ifile][iat][ipt][iht]);
 	  }
	  ...
	  ss_axis << "R(#alpha_{T}=" << at[iat]/1000. << ")";
 	  ...
 	  histo->GetYaxis()->SetRangeUser(min[iat]/2.,max[iat]*2.);
 	  histo->GetXaxis()->SetTitle("p_{T}^{min} [GeV]");
 	  histo->GetYaxis()->SetTitle(TString(ss_axis.str()));
 	  histo->SetMarkerStyle(style[ifile]);
 	  //histo->SetMarkerSize(1.5);
 	  histo->SetMarkerColor(iht+1);
 	  //histo->SetBarOffset(10.*ipt);
	  
 	  if ( ifile == 0 && iht == 0 ) { histo->Draw(""); }
 	  else { histo->Draw("same"); }
	  
	  std::stringstream ss_legend;
	  ss_legend << type[ifile] << " HT_{reco} = " << ht[iht];
	  legend->AddEntry( histo, TString(ss_legend.str()), "lep" );
      ...
      canvas->SaveAs(TString("RatioVsPt_"+ss_canvas.str()+".png")); 
