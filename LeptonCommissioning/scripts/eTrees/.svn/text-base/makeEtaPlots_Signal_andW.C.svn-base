...
TFile *fSig=new TFile("eWPol_Signal_Odd_SD_JMT_SKMD.root");
TFile *fCtrl=new TFile("eWPol_Signal_MC_QCD15P8_SKMD.root");
TFile *fW = new TFile("eWPol_Signal_WJets-madgraph.root");

TString file = "MCpWvsJMT_Signal";
...
fit_info* draw_tree(double cut, double step) {
  ...
  TString theEta=" && caloIso<0.3";// theEta+=cut;
  theEta+=" && eta_j1>"; theEta+=cut;
  theEta+=" && eta_j1<"; theEta+=(cut+step); 

  TH1D *hSelection=plotIso(tSig,"alphaT",weight,etCut+" && nEle==1",theEta,
			   "nJet>=1 && et_j1>40.","sel","(100,0.,2.)",1.,1);
  TH1D *hControl=plotIso(tCtrl,"alphaT",weight,etCut+" && nEle==1",theEta,
			   "nJet>=1 && et_j1>40.","ctrl","(100,0.,2.)",1.,3);
  TH1D *hConversions=plotIsoConv(tSig,"alphaT",weight,etCut+" && nEle==1",theEta,
			   "nJet>=1 && et_j1>40.","conv","(100,0.,2.)",1.,2);
 TH1D *hW=plotIso(tW,"alphaT",weight,etCut+" && nEle==1",theEta,
			   "nJet>=1 && et_j1>40.","w","(100,0.,2.)",1.,1);
...
fit_info* do_fit(TH1D *hSelection, TH1D *hControl, TH1D *hConversions, double theHT, double thestep) {
  ...
  // Fitting
  //  double fitH=GetMinimum(hSelection,0.55,3.);  RooFitResult *fitr=0;// = bkg.fitTo(data,Range(0.3,fitH),Minos(kTRUE),Extended(kTRUE), Save(kTRUE));
  ...
  TLegend *leg = new TLegend(.5,.8,.99,.99,"");
  ...
  ///TString headerNamea ="ele ID inversion anti-selected events for "; headerNamea +=etalow;
    TString headerNameb = " < |#eta_{j1}| < "; headerNameb+=etahigh;
    TString varcut =variable; varcut+=theHT; varcut+="_to_"; varcut+=(theHT+thestep);
  ...
  // xframe->GetYaxis()->SetRangeUser(0.0001,hSelection->GetMaximum()+100.);
  xframe->Draw();
...
void makeEtaPlots_Signal_andW() {
  ...
  //  for (double nj=40.; nj<=150.; nj+=15.) { // HT bins
    for (double nj=0; nj<=3.0; nj+=step) { // j1 eta bins
...
//  leg->AddEntry(g2,"predicted (W+QCD)","P");
 ...
 g1->SetTitle(";H_{T} cut;RaT = f(#alpha_{T}>0.55)");
  g1->Draw("AP");
