...
fit_info* draw_tree(double cut, double step) {
  ...
  fit_info *fit = do_fit(hSelection,hControl,hConversions,cut);
  return fit;
...
fit_info* do_fit(TH1D *hSelection, TH1D *hControl, TH1D *hConversions, double theHT) {
  ...
  leg->AddEntry(hControl,"eleID anti-selected events","L");
...
void fit_aT() {
  ...
  leg->AddEntry(g3,"Selection region","P");
  leg->AddEntry(g1,"Control region (ele ID anti-selection)","P");
//  leg->AddEntry(g2,"predicted (W+QCD)","P");
  ...
  //g2->Draw("AP");
  g1->SetTitle(";H_{T} cut;RaT = #frac{N(aT>0.55)}{N(aT>0)}");
  g1->Draw("AP");
