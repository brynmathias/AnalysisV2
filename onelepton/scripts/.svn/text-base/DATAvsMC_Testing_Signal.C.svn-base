...
// SET PLOT PARAMETERS ==============================
  ...
  Parameters.ShowStatErrorbars = false;
...
// Create Canvases and Plots
  ...
  for(unsigned int i=0; i<folderNames.size(); i++) {
    if(plots.count("LP_pos")>0) { 
      createDataVsMC(folderNames[i],"LP_pos",2*rebinFactor[i],-0.5,1.5,"L_{P}[+]","Entries / bin",MCFileNames,MCNames,MCColors,AddToBkg,dataFileNames,dataNames,dataColors,SystNames,SystColors,Canvases[plotNr/Parameters.HistosPerCanvas],plotNr%Parameters.HistosPerCanvas,false,Parameters);
      plotNr++;
    } 
    if(plots.count("LP_neg")>0) {    	
      createDataVsMC(folderNames[i],"LP_neg",2*rebinFactor[i],-0.5,1.5,"L_{P}[-]","Entries / bin",MCFileNames,MCNames,MCColors,AddToBkg,dataFileNames,dataNames,dataColors,SystNames,SystColors,Canvases[plotNr/Parameters.HistosPerCanvas],plotNr%Parameters.HistosPerCanvas,false,Parameters);
      plotNr++;
    } 
  ...
  for(unsigned int i=0; i<folderNames.size(); i++) {
    ...
    if(plots.count("MT_tot")>0) {
      createDataVsMC(folderNames[i],"MT_tot",5*rebinFactor[i],0,500,"MT","Entries / bin",MCFileNames,MCNames,MCColors,AddToBkg,dataFileNames,dataNames,dataColors,SystNames,SystColors,Canvases[plotNr/Parameters.HistosPerCanvas],plotNr%Parameters.HistosPerCanvas,false,Parameters);
      plotNr++;
    }
  }
  for(unsigned int i=0; i<folderNames.size(); i++) { 
    if(plots.count("SumLepPt_pos")>0) {
      createDataVsMC(folderNames[i],"SumLepPt_pos",20*rebinFactor[i],0,1000,"ST_{lep}[+]","Entries / bin",MCFileNames,MCNames,MCColors,AddToBkg,dataFileNames,dataNames,dataColors,SystNames,SystColors,Canvases[plotNr/Parameters.HistosPerCanvas],plotNr%Parameters.HistosPerCanvas,false,Parameters);
      plotNr++;
    } 
    ...
    if(plots.count("SumLepPt_tot")>0) {
      createDataVsMC(folderNames[i],"SumLepPt_tot",20*rebinFactor[i],0,1000,"ST_{lep}","Entries / bin",MCFileNames,MCNames,MCColors,AddToBkg,dataFileNames,dataNames,dataColors,SystNames,SystColors,Canvases[plotNr/Parameters.HistosPerCanvas],plotNr%Parameters.HistosPerCanvas,false,Parameters);
      plotNr++;
    }
  }
  for(unsigned int i=0; i<folderNames.size(); i++) { 
  ...
  for(unsigned int i=0; i<folderNames.size(); i++) { 
    if(plots.count("LepPt_pos")>0) {
      createDataVsMC(folderNames[i],"LepPt_pos",25*rebinFactor[i],0,500,"LepPt[+]","Entries / bin",MCFileNames,MCNames,MCColors,AddToBkg,dataFileNames,dataNames,dataColors,SystNames,SystColors,Canvases[plotNr/Parameters.HistosPerCanvas],plotNr%Parameters.HistosPerCanvas,false,Parameters);
      plotNr++;
    } 
    if(plots.count("LepPt_neg")>0) {
      createDataVsMC(folderNames[i],"LepPt_neg",25*rebinFactor[i],0,500,"LepPt[-]","Entries / bin",MCFileNames,MCNames,MCColors,AddToBkg,dataFileNames,dataNames,dataColors,SystNames,SystColors,Canvases[plotNr/Parameters.HistosPerCanvas],plotNr%Parameters.HistosPerCanvas,false,Parameters);
      plotNr++;
    } 
    if(plots.count("LepPt_tot")>0) {
      createDataVsMC(folderNames[i],"LepPt_tot",25*rebinFactor[i],0,500,"LepPt","Entries / bin",MCFileNames,MCNames,MCColors,AddToBkg,dataFileNames,dataNames,dataColors,SystNames,SystColors,Canvases[plotNr/Parameters.HistosPerCanvas],plotNr%Parameters.HistosPerCanvas,false,Parameters);
      plotNr++;
    }
  }
  for(unsigned int i=0; i<folderNames.size(); i++) { 
...
TH1D *plot1Dhisto(double intLumi,TFile *fileName,TString folderName,TString histoName,int color,int rebin,float xMin,float xMax,TString xName, TString yName,TString sampleName,bool mc) {  
  ...
  if ((color==1) && (sampleName=="MC")) { hTemp->SetLineStyle(kDashed); }
...
void createDataVsMC(TString folderName,TString histoName,int rebin,double xMin,double xMax,TString xName, TString yName, vector<TString> MCFileNames, vector<TString> MCNames, vector<int> MCColors, vector<bool> AddToBkg,vector<TString> DataFileNames,vector<TString> dataNames,vector<int> dataColors,vector<TString> SystNames,vector<int> SystColors, TCanvas *cCanvas, int padNr, bool UseLog,GlobalParameterSet Parameters) {
  ...
  for(int i=dataHistos.size()-1;i>=0;i--) {
    dataHistos[i]->Draw("HIST E0 same");
  }
  if(Parameters.ShowMCStatErrorbars&&!Parameters.StackMCComposition) {
  ...
  for(unsigned int i=0; i<dataHistos.size()&&MChistos.size()>0; i++) {
    ...
    hRatio.back()->GetYaxis()->SetTitleSize(0.055);
...
TLegend *legendRAW() {
  ...
  TLegend *leg = new TLegend(0.67,0.55,0.95,0.93);
  leg->SetFillStyle(0);
  leg->SetBorderSize(0);
