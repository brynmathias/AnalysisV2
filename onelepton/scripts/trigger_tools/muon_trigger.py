...
MuonPlots = LeptonicPlots("MuonStandardPlots",mu_plots_cfg.ps())
MuonPlotsHLT = LeptonicPlots("MuonStandardPlots_PostHLT",mu_plots_cfg.ps())

triggerCUT = OP_MultiTrigger(trg_set5.ps())

# muon specific plots/trees END
sig, tree, extra = setupMuonPreselection(MET,"Muons", mode, True)

tree.TAttach(PFMuonCheck,RECO_CommonHTCut350)
tree.TAttach(RECO_CommonHTCut350,MuonPlots)
tree.TAttach(MuonPlots,triggerCUT)
tree.TAttach(triggerCUT,MuonPlotsHLT)
...
samplesList = {
    ...
    "data42X_PromptReco" : samplesData42X_SingleMu_PromptReco,
    "data42X_ReReco"     : samplesData42X_SingleMu_ReReco,
    }
...
output_dir = autoname("./resultsTrigger%(syst_mode)s", syst_mode=syst_str)
