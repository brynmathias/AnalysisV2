...
TString plotting = "~/Plots/RaT/err1/";
...
TCut a_j1pt = "JetPt[1]>100.";
TCut b_j1pt = "JetPt[1]>85.8";
TCut c_j1pt = "JetPt[1]>71.4";
...
TCut numpj = numlep && npho && npho && nj && jodd && bmu && jeta1 && jeta2;
TCut numpjData = numlepData && npho && nj && jodd && bmu && jeta1 && jeta2;
TCut ht_250 = "HT_Lep > 250. && HT_Lep <= 280.";
TCut ht_300= "HT_Lep > 280. && HT_Lep <= 350.";
TCut ht_350 = "HT_Lep > 350.";

TCut reg1 = numpj && ht_250 && a_j1pt;
TCut reg2 = numpj && ht_300 && b_j1pt;
TCut reg3 = numpj && ht_350 && c_j1pt;
...
//TFile *sfSUS = new TFile("muAlphaT_"+selec+"SUSLM0.root");
...
//TFile *asfQCD = new TFile("muAlphaT_"+aselec+"QCD_AllPtBins_7TeV_Pythia.root");
TFile *asfW = new TFile("muAlphaT_"+aselec+"Wjets_vols.root");
TFile *asfZ = new TFile("muAlphaT_"+aselec+"ZJets_madgraph.root");
//TFile *asfData = new TFile("muAlphaT_"+aselec+"Data.root");
...
Double_t intlumi = 35.0;
TString luminum = "35.0";
//wgt is 10 - times this by required lumi/10 e.g. 10*wgt = 100;
TString MClumifactor = "10*wgt";

void mu_RaT(){

 
  //Switch these on for 4x SAS plots at each cut stage
  ...
  */
  ...
  //This does the HT stage's in one plot. Change the stSUS to other files to investigate e.g. stData, or stSM
  //   HT_RaTPlot(stSUS,"Signal","SUSY");


HTRegionsRaT();
    //These NaT give the numerical upper bound method reuslts 
  /* 
     NaT(stData,astData,trigMC+numpj,"noHT");
  NaT(stData,astData,trigMC+HT200,"HT>200");
  NaT(stData,astData,trigMC+HT300,"HT>300");
   NaT(stData,astData,trigMC+HT350,"HT>350");
  */ 
}
void HTRegionsRaT(){
...
TGraphErrors *data51 = new TGraphErrors(3);
 ...
 Double_t a51_SM_e_350 = NumRaTSMErr(reg3,51);
  ...
  data ->SetPoint(0,265.,data_250);
  data ->SetPointError(0,15.,data_e_250);
  data ->SetPoint(1,310.,data_300);
  data ->SetPointError(1,30.,(data_e_300));
  data ->SetPoint(2,375.,data_350);
  data ->SetPointError(2,25.,(data_e_350));

  SM->SetPoint(0,265.,SM_250);
  SM->SetPointError(0,15.,(SM_e_250));
  SM->SetPoint(1,310.,SM_300);
  SM->SetPointError(1,30.,(SM_e_300));
  SM->SetPoint(2,375.,SM_350);
  SM->SetPointError(2,25.,(SM_e_350));

data51->SetPoint(0,265.,a51_data_250);
  data51->SetPointError(0,15.,a51_data_250);
  data51->SetPoint(1,295.,a51_data_300);
  data51->SetPointError(1,15.,a51_data_300);
  data51->SetPoint(2,335.,a51_data_350);
  data51->SetPointError(2,15.,a51_data_350);

  SM51->SetPoint(0,265.,a51_SM_250);
  SM51->SetPointError(0,15.,a51_SM_e_250);
  SM51->SetPoint(1,295.,a51_SM_300);
  SM51->SetPointError(1,15.,a51_SM_e_300);
  SM51->SetPoint(2,335.,a51_SM_350);
  SM51->SetPointError(2,15.,a51_SM_e_350);
...
Double_t NumRaTSM(TCut region, UInt_t aT){
  ...
  TH1D* QCD = GetaTHist(stQCD, region, "QCD");
...
TCanvas *c7 = new TCanvas ("SAS"+point+set,"SAS"+point+set);
 ...
 leg->Draw("SAME");
 c7->Update();
 c7->SaveAs(plotting+"RaT_"+set+"_"+point+".png");
...
TCanvas *c7 = new TCanvas ("SAS"+sel+set,"SAS"+sel+set);
 ...
 noHT->SetTitle();
 noHT->GetXaxis()->SetTitle("|#eta | leading jet");
 noHT->GetYaxis()->SetTitle("N(above aT 0.55) / N(all)");
 noHT->GetYaxis()->SetTitleOffset(1.2);
 noHT->GetYaxis()->SetRangeUser(0.001,0.1);

 leg->Draw("SAME");
 c7->Update();
 c7->SaveAs(plotting+"MUON_RaT_HTbins_"+set+"_"+sel+".png");
...
TH1D* GetHist(TString dataset, TString flow,TTree *t, TString poin, TString type){
 ...
 t->Draw("JetEta[0]>>hist1(6,0.,3.)",MClumifactor+"*("+flow+")");
 ...
 TCanvas *c2= new TCanvas("canny2","canne2",400,400);
  t->Draw("-1*JetEta[0]>>hist2(6,0.,3.)",MClumifactor+"*("+flow+")");
 ...
 TCanvas *c1 = new TCanvas("canny1","canne1",400,400);
 t->Draw("JetEta[0]>>hist1(6,0.,3.)",MClumifactor+"*("+flow+" && AlphaT_Lep > 0.55)");
 TH1D *HIST1= (TH1D*)gDirectory->Get("hist1");
  HIST1->SetName("one_"+dataset+"JetEtaFabs"+poin+type);
 c1->Close();

  TCanvas *c2= new TCanvas("canny2","canne2",400,400);
  t->Draw("-1*JetEta[0]>>hist2(6,0.,3.)",MClumifactor+"*("+flow+" && AlphaT_Lep > 0.55)");
  TH1D *HIST2= (TH1D*)gDirectory->Get("hist2");
   HIST2->SetName("two_"+dataset+"JetEtaFabs"+poin+type);
  c2->Close();
