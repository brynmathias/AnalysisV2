...
TGraph* set_tev_stau(Int_t tanBeta){
    ...
    double st_m0_tanBeta40[]={240, 260, 280, 300, 320, 340, 360, 380, 400, 420, 440, 460, 480, 500, 520, 540, 560, 580, 600, 620, 640, 660, 680, 700, 720, 740, 760, 780, 800, 820, 840, 860, 880, 240};
    double st_m12_tanBeta40[]={186.04449, 256.69360, 329.02451, 400.61670, 470.30460, 537.94281, 603.24860, 666.41870, 727.58569, 787.32202, 845.57990, 902.33801, 958.58685, 1013.3401, 1067.5797, 1121.3193, 1174.5591, 1226.7979, 1278.5367, 1330.2756, 1381.1019, 1431.9282, 1481.8265, 1531.7249, 1581.6232, 1630.5215, 1679.4198, 1728.4783, 1779.5099, 1825.5411, 1874.4442, 1920.2191, 1971.9650, 1971.9650};
    ...
    TGraph* st_gr_tanBeta40 = new TGraph(34,st_m0_tanBeta40,st_m12_tanBeta40);
...
//---lines of constant gluino/squark
  ...
  TF1* lnsq;
  if (tanBeta == 10) lnsq = new TF1(hname,"sqrt([0]-x*x*[1]+[2])",0,2000);
  if (tanBeta == 40) lnsq = new TF1(hname,"sqrt([0]-x*x*[1]+[2])",380,2000);
...
//---lines of constant gluino/squark
  ...
  TF1* lngl;
  if (tanBeta == 10) lngl = new TF1(hname,"[0]+x*[1]",0,2000);
  if (tanBeta == 40) lngl = new TF1(hname,"[0]+x*[1]",380,2000);
...
//   //cout << "Hello, still alive " << endl; 

//   double mass2[]={mass};
//   massGrid.SetContour(1,mass2);
//   massGrid.Draw("CONT LIST");
//   gPad->Update();

//   TObjArray *contours = gROOT->GetListOfSpecials()->FindObject("contours");
//   Int_t ncontours     = contours->GetSize();
//   TList *list         = (TList*)contours->At(0);
//   TGraph *theGraph = (TGraph*)list->First();

//   theGraph->SetLineWidth(1);
//   theGraph->SetLineColor(kGray);
//   //cout << "now dead" << endl;
//   //theGraph->Print();
//   //  cout << "Graph printing done " << endl;
//   //  theGraph->Draw("Psame");
...
TGraph* constant_mass(double mass, TH2F& massGrid)
{
  ...
  TObjArray *contours = gROOT->GetListOfSpecials()->FindObject("contours");
  Int_t ncontours     = contours->GetSize();
  cout << ncontours << " " << levels[] << endl;
  //for (int i=0;i<ncontours;i++) {
    TList *list = (TList*)contours->At(0);
    theGraph = (TGraph*)list->First();
    theGraph->SetLineWidth(1);
    theGraph->SetLineColor(kGray);
    //}
...
TLatex* constant_squark_text(Int_t it,TF1& lnsq,Int_t tanBeta_){
  ...
  TLatex* t3 = new TLatex(place_x+10*it,lnsq.Eval(place_x+50)+it*8,legnm);
  t3->SetTextSize(0.03);
...
//// Needs to be modified
    ...
    TGraph* st_gr;
    if (tanBeta ==10) st_gr = new TGraph(25,st_m0_tanBeta10,st_m12_tanBeta10);
    if (tanBeta ==40) st_gr = new TGraph(25,st_m0_tanBeta40,st_m12_tanBeta40);
