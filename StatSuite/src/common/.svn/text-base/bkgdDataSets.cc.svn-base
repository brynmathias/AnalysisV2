...
void bkgdDataSets::mergePlots(){
  ...
  // std::cout << " in merge plots  " << std::endl;
  ...
  //cout << " signalUpbin " << signalUpbin << " bkgLowbin " << bkgdLowbin << endl;
  ...
  for (vector<TFile*>::iterator tfileIt = dataFileVec.begin();tfileIt!=dataFileVec.end();tfileIt++)
    {    
      ...
      // cout << " TH2DPlotName " << TH2DPlotName << endl;
  ...
  for(int i =1; i <= var1bins; i++){
    // cout << " var1 bin " << i << " bin low edge " << AddedSignal->GetBinLowEdge(i) << " pass " << AddedSignal->GetBinContent(i) << " fail " << AddedBkgd->GetBinContent(i) << endl;
    pass.push_back(AddedSignal->GetBinContent(i));
    fail.push_back(AddedBkgd->GetBinContent(i));
  }
  //cout << " end of put in " << endl;
