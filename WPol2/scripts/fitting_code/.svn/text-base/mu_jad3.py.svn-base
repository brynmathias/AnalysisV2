...
#fit templates to systematically shifted mht scale as a function of fitting range

if __name__ == "__main__":
    mu_path = "./ROOTFiles/"
    mu_file_names = ["RecoRoutines_W-selection_realdata.root",
                     "RecoRoutines_W-selection_WJets_madgraph_June2010_shift_pfmht_p3pc.root",
    ...
    # Read W templates
    mu_plus_wtemplates = [mu_files[3].Get("RECO_PolPlots_"+ptwbin).Get("RECO_ICVarPFPlus_%s" % s)
                          for s in ["LH", "RH", "LO"]]
    mu_minus_wtemplates = [mu_files[3].Get("RECO_PolPlots_"+ptwbin).Get("RECO_ICVarPFMinus_%s" % s)
                          for s in ["LH", "RH", "LO"]]
    ...
    #plus/minus 0.1 to these
    xmin_plus = -0.5
    xmax_plus = 1.5
    xmin_minus = -0.4
    xmax_minus = 1.6

    flfr_mhtshiftp3pcvalues_histminus = r.TH1D("flfr_mhtshiftp3pcvalues_histminus","flfr_mhtshiftp3pcvalues_histminus",30,-0.05,2.95)
    f0_mhtshiftp3pcvalues_histminus = r.TH1D("f0_mhtshiftp3pcvalues_histminus","f0_mhtshiftp3pcvalues_histminus",30,-0.05,2.95)
    flfr_mhtshiftp3pcvalues_histplus = r.TH1D("flfr_mhtshiftp3pcvalues_histplus","flfr_mhtshiftp3pcvalues_histplus",30,-0.05,2.95)
    f0_mhtshiftp3pcvalues_histplus = r.TH1D("f0_mhtshiftp3pcvalues_histplus","f0_mhtshiftp3pcvalues_histplus",30,-0.05,2.95)
    ...
    for index in range(5):
      ...
      flfr_mhtshiftp3pcvalues_histplus.Fill(fit_range_size,fLfR_resultplus[0])
      flfr_mhtshiftp3pcvalues_histplus.SetBinError(flfr_mhtshiftp3pcvalues_histplus.FindBin(fit_range_size),fLfR_resultplus[1]);
      f0_mhtshiftp3pcvalues_histplus.Fill(fit_range_size,f0_resultplus[0])
      f0_mhtshiftp3pcvalues_histplus.SetBinError(f0_mhtshiftp3pcvalues_histplus.FindBin(fit_range_size),f0_resultplus[1]);

      flfr_mhtshiftp3pcvalues_histminus.Fill(fit_range_size,fLfR_resultminus[0])
      flfr_mhtshiftp3pcvalues_histminus.SetBinError(flfr_mhtshiftp3pcvalues_histminus.FindBin(fit_range_size),fLfR_resultminus[1]);
      f0_mhtshiftp3pcvalues_histminus.Fill(fit_range_size,f0_resultminus[0])
      f0_mhtshiftp3pcvalues_histminus.SetBinError(f0_mhtshiftp3pcvalues_histminus.FindBin(fit_range_size),f0_resultminus[1]);
    ...
    outputfile = r.TFile("outputfile_mhtshiftp3pcvalues50toinf.root", "RECREATE")
    flfr_mhtshiftp3pcvalues_histplus.Write()
    f0_mhtshiftp3pcvalues_histplus.Write()
    flfr_mhtshiftp3pcvalues_histminus.Write()
    f0_mhtshiftp3pcvalues_histminus.Write()
    outputfile.Close()
