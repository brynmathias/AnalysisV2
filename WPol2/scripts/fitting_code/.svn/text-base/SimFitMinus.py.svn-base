...
mu = run("mu_minus_data",
         fit_globals,
         mu_real_minus_hists,
         mu_minus_templates,
         {"w" : mu_wfraction_minus},
         action = "fit_minuit",
          **cfg
         )
contour.plot_contour(fit_globals, mu, 1, "./Plots/mu_minus_contours/1.pdf")
...
# Fit el_plus (Data)
el = run("el_minus_data",
          fit_globals,
          el_real_minus_hists,
          el_minus_templates,
          {"w" : el_wfraction_minus,
           "qcd" : el_qcdfraction_minus},
          # constrain_qcd = True,
...
fit = sim_fit("minus_fit",
              fit_globals,
              "ele_minus_data",
              ele_minus_data = el,
              muon_minus_data = mu
              )
contour.plot_contour(fit_globals, fit, 1, "./Plots/sim_minus_contours/1.pdf")
fit["result"].Print("v")
fit["sim_pdf"].Print("v")
