...
TString dir="../results10/";
 // input data Files
TFile *sigfile = new TFile(dir+"reco_mc/RecoRoutines_Z-selection_DYJetsToLL_TuneZ2_M-50_7TeV-madgraph-tauola.Spring11-PU_S1_START311_V1G1-v1.AODSIM.root");
//TFile *sigfile = new TFile(dir+"reco_mc/RecoRoutines_Z-selection_ZJets_TuneZ2_7TeV_alpgen_tauola.root");

TFile *bkgfile=new TFile(dir+"reco_mc/RecoRoutines_Z-selection_TTJets_TuneZ2_7TeV_madgraph_tauola_Spring11_PU_S1_START311_V1G1_v1.root"); 
TFile *datafile = new TFile(dir+"reco_data/data.root");
//TFile *datafile = new TFile(dir+"reco_data/RecoRoutines_Z-selection_SingleMu_Run2011A_May10ReReco_v1_muonphys_skim.root");
//TFile *datafile = new TFile(dir+"reco_data/RecoRoutines_Z-selection_SingleMu_Run2011A_PromptReco_v4_muonphys_skim.root");
//TFile *datafile = new TFile(dir+"reco_data/RecoRoutines_Z-selection_SingleMu_uptoRun163817.root");
//TFile *datafile = new TFile(dir+"reco_mc/RecoRoutines_Z-selection_DYJetsToLL_TuneZ2_M-50_7TeV-madgraph-tauola.Spring11-PU_S1_START311_V1G1-v1.AODSIM.root");
//TFile *datafile = new TFile(dir+"reco_mc/RecoRoutines_Z-selection_ZJets_TuneZ2_7TeV_alpgen_tauola.root");
...
void roo_fitZH()
{
  ...
  TString canvas_name_plus = "MC_ZHelicityFramePlots_PlusCosThetaStar";
  ...
  for(unsigned int j=0; j<charge.size(); j++) {
    ...
    if (j==1) break; // do only + charge fit
      ...
      // Nominal values are taken from a fit to the MC Z data (scaled to 100pb-1)
...
// RICALCOLARE
    ...
    TString Hist1 = "RECO_PolPlots_50toinf/RECO_CosThetaStar1Sol1Corr"+ charge.at(j) + "_LH";
    TString Hist2 = "RECO_PolPlots_50toinf/RECO_CosThetaStar1Sol1Corr"+ charge.at(j) + "_RH";
    TString Hist3 = "RECO_PolPlots_50toinf/RECO_CosThetaStar1Sol1Corr"+ charge.at(j) + "_LO";
    TString Hist_data1 = "RECO_PolPlots_50toinf/RECO_CosThetaStar1Sol1Corr"+ charge.at(j)+"";
    
    TH1D *mc1 = (TH1D*)sigfile->Get(Hist1);
    TH1D *mc2 = (TH1D*)sigfile->Get(Hist2);
    TH1D *mc3 = (TH1D*)sigfile->Get(Hist3);
    
    TH1D *sighist = (TH1D*)sigfile->Get(Hist_data1); // Z signal histogram
...
hdata->Scale(sfactor);
    ...
    Double_t f_bkg=nbkg/sighist->Integral(datahist->FindBin(xmin+quanta),datahist->FindBin(xmax-quanta)); // bkg fraction

      // Start RooFit session
    RooRealVar x("x","CosThetaStar",xmin,xmax);
      // Import binned Data
    RooDataHist data1("data1","dataset with CosThetaStar",x,mc1);
    RooDataHist data2("data2","dataset with CosThetaStar",x,mc2);
    RooDataHist data3("data3","dataset with CosThetaStar",x,mc3);
  
    RooDataHist test("data","dataset with CosThetaStar",x,datahist);
    RooDataHist data_bkg("data4","dataset with CosThetaStar",x,bkghist);
    ...
    if (charge.at(j) == "Plus") { // plus charge PDFs
      if (addParameter) { sig = new RooZPlusExtended("sigmodel","model",x,f1,f2,fsub);
      } else { sig = new RooZPlus("sigmodel","model",x,f1,f2); } 
  ...
  if(charge.at(j) == "Plus") {  hdata->GetXaxis()->SetTitle("CosThetaStar(#mu^{+})");}
  if(charge.at(j) == "Minus") {hdata->GetXaxis()->SetTitle("L_{P}(#mu^{-})");}
      ...
      hdata->GetXaxis()->SetRangeUser(-1,1);
  //    hdata->GetYaxis()->SetRangeUser(0.0,100);
      ...
      hdata->Draw("");
      //  frame->Draw("same");
      mc1->GetXaxis()->SetRangeUser(-1,1);
      float f_l_a =  mc1->Integral();
      mc2->GetXaxis()->SetRangeUser(-1,1);
      float f_r_a =  mc2->Integral();
      mc3->GetXaxis()->SetRangeUser(-1,1);
      ...
      if(addbkg) bkghist->Draw("samehist");
      ...
      allHists->Add(mc2,1);
      allHists->Add(mc3,1);
      if (addbkg) allHists->Add(bkghist,1);
	...
	//  if(charge.at(j) == "Plus") 
aleg = new TLegend(0.8,0.69,0.95,0.9,"");
      aleg->SetFillColor(0);
      aleg->SetLineColor(0);
      aleg->AddEntry(mc1,"f_{L}","lpf");
      aleg->AddEntry(mc2,"f_{R}","lpf");
      aleg->AddEntry(mc3,"f_{0}","lpf");
      aleg->AddEntry(bkghist,"EWK","lpf");
	...
	preliminary = new TLatex(0.05,550,"CMS preliminary: #it{L} = 36pb^{-1}@ 7 TeV");}
