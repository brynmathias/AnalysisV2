void getViZ() 
{
  TString Hist1 = "MC_WPlots_50toinf/MC_CosThetaStar1Plus";
  // TString Hist2 = "MC_WPlots_75to100/MC_CosThetaStar1Plus";
  // TString Hist3 = "MC_WPlots_100toinf/MC_CosThetaStar1Plus";
  
TFile *data1 = new TFile("./GenRoutines_ZJets_TuneZ2_7TeV_alpgen_tauola.rew8.corr1.root");
...
//  TF1 * f2 = new TF1("f2","[2]*((1-x)*(1-x)/4*[0] + (1+x)*(1+x)/4*[1] + (1.0-[0]-[1])*(1-x*x)/2)",-0.99,1.00);
  TF1 * f2 = new TF1("f2","[2]*((1-x)*(1-x)/4*(1+[0]-[1])/2 + (1+x)*(1+x)/4*(1-[0]-[1])/2 + [1]*(1-x*x)/2)",-0.99,1.00);
  ...
  //Cos Theta* fits:
  std::cout << "Fit for ZPt 50-inf, Plus Charge, Cos Theta*: " << std::endl;
  ...
  cout << "f1 = " << f2->GetParameter(0) << " +/- " << f2->GetParError(0) << endl;
  cout << "f2 = " << f2->GetParameter(1) << " +/- " << f2->GetParError(1) << endl;

 /* cout << "f0 = " << 1-f2->GetParameter(0)-f2->GetParameter(1) << " +/- " << sqrt(cov[0][0]+cov[1][1]+2.0*cov[0][1]) << endl;
